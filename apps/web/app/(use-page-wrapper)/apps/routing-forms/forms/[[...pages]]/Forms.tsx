...
function NewFormButton({ setNewFormDialogState }: { setNewFormDialogState: SetNewFormDialogState }) {
  ...
  return (
    <Button
      ...
      onClick={() => {
        setNewFormDialogState({ action: "new", target: "" });
        posthog.capture("new_routing_form_button_clicked", { teamId: null });
      }}>
      {t("create_routing_form")}
    </Button>
  );
}
...
export default function RoutingForms({ appUrl }: { appUrl: string }) {
  ...
  return (
    <ShellMain
      ...
      heading={t("routing")}
      CTA={
        hasPaidPlan && forms?.length ? <NewFormButton setNewFormDialogState={setNewFormDialogState} /> : null
      }
