...
type DelegatedMember = {
  userId: number;
};

type DelegationCredentialWithDelegatedMembers = {
  id: string;
  workspacePlatform: { slug: string };
  organization: {
    id: number;
...
async function findAllEnabledIncludeDelegatedMembers(): Promise<DelegationCredentialWithDelegatedMembers[]> {
  const credentials = await prisma.delegationCredential.findMany({
    where: { enabled: true },
    select: {
      id: true,
      workspacePlatform: {
  ...
  return credentials.map((credential) => ({
    id: credential.id,
    workspacePlatform: credential.workspacePlatform,
    organization: {
      id: credential.organization.id,
      delegatedMembers: credential.organization.members,
    },
  }));
}

async function findAllDisabledAndIncludeNextBatchOfMembersToProcess() {
...
export async function handleCreateCredentials() {
  ...
  const delegationCredentials = await findAllEnabledIncludeDelegatedMembers();
...
async function handleDeleteCredentials() {
  ...
  const delegationCredentials = await findAllDisabledAndIncludeNextBatchOfMembersToProcess();
