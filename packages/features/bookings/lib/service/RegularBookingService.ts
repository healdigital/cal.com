...
async function handler(
      ...
      for (const [groupId, luckyUserPool] of Object.entries(luckyUserPools)) {
        ...
        while (luckyUserPool.length > 0 && !luckUserFound) {
          ...
          const newLuckyUser = await deps.luckyUserService.getLuckyUser({
            ...
            ) as Parameters<LuckyUserService["getLuckyUser"]>[0]["allRRHosts"],
            eventType,
            routingFormResponse,
  ...
  */
  ...
  const credentials = await refreshCredentials(allCredentials as CredentialForCalendarService[]);
  const apps = eventTypeAppMetadataOptionalSchema.parse(eventType?.metadata?.apps);
  const eventManager =
    !isDryRun && !skipCalendarSyncTaskCreation
      ? new EventManager({ ...organizerUser, credentials: credentials as CredentialForCalendarService[] }, apps)
  ...
  if (!eventType.seatsPerTimeSlot && originalRescheduledBooking?.uid) {
    ...
    if (changedOrganizer && originalRescheduledBooking?.user) {
      ...
      const refreshedOriginalHostCredentials = await refreshCredentials(originalHostCredentials as CredentialForCalendarService[]);
      ...
      const originalHostEventManager = new EventManager(
        { ...originalRescheduledBooking.user, credentials: refreshedOriginalHostCredentials as CredentialForCalendarService[] },
        apps
      );
  ...
  if (!isDryRun) {
    await handleAnalyticsEvents({
      credentials: allCredentials as CredentialForCalendarService[],
      rawBookingData,
      bookingInfo: {
        name: fullName,
        email: bookerEmail,
