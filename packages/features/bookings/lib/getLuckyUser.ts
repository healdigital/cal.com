...
export class LuckyUserService implements ILuckyUserService {
  ...
  private leastRecentlyBookedUser<T extends PartialUser>({
    ...
    // Build email→userId lookup for O(1) matching instead of O(users × attendees) per booking
  ...
  private getHostsWithCalibration({
    ...
    // Sort bookings by createdAt once for binary search — O(n log n)
    const sortedBookings = [...existingBookings].sort(
      (a, b) => a.createdAt.valueOf() - b.createdAt.valueOf()
    );
    const sortedTimestamps = sortedBookings.map((b) => b.createdAt.valueOf());
    ...
    // OOO calibration: use binary search for range counting — O(oooEntries × log n) instead of O(oooEntries × n)
    ...
    oooData.forEach(({ userId, oooEntries }) => {
      ...
      oooEntries.forEach((oooEntry) => {
        const startIdx = lowerBound(oooEntry.start.valueOf());
        const endIdx = upperBound(oooEntry.end.valueOf());
        // Count bookings in range excluding this user
        let count = 0;
        for (let i = startIdx; i < endIdx; i++) {
          if (sortedBookings[i].userId !== userId) {
            count++;
          }
        }
        calibration += count / (hosts.length - 1);
      });

      oooCalibration.set(userId, calibration);
    });

    // New host calibration: use binary search + pre-sorted hosts — O(newHosts × log n)
    // Sort hosts by createdAt for binary search
    const sortedHosts = [...hosts].sort((a, b) => a.createdAt.valueOf() - b.createdAt.valueOf());
    const sortedHostTimestamps = sortedHosts.map((h) => h.createdAt.valueOf());

    function calculateNewHostCalibration(newHost: { userId: number; createdAt: Date }) {
      const cutoff = newHost.createdAt.valueOf();

      // Count bookings created before this host (excluding their own)
      const bookingEndIdx = lowerBound(cutoff);
      let bookingsBeforeCount = 0;
      for (let i = 0; i < bookingEndIdx; i++) {
        if (sortedBookings[i].userId !== newHost.userId) {
          bookingsBeforeCount++;
        }
      }

      // Count hosts added before this host (excluding themselves) using binary search
      let hostCutoffIdx = 0;
      {
        let lo = 0;
        let hi = sortedHostTimestamps.length;
        while (lo < hi) {
          const mid = (lo + hi) >>> 1;
          if (sortedHostTimestamps[mid] < cutoff) lo = mid + 1;
          else hi = mid;
        }
        hostCutoffIdx = lo;
      }
      let hostsBeforeCount = 0;
      for (let i = 0; i < hostCutoffIdx; i++) {
        if (sortedHosts[i].userId !== newHost.userId) {
          hostsBeforeCount++;
        }
      }

      const calibration =
        bookingsBeforeCount && hostsBeforeCount ? bookingsBeforeCount / hostsBeforeCount : 0;
      log.debug(
        "calculateNewHostCalibration",
        safeStringify({
          newHost,
          existingBookingsBeforeAdded: bookingsBeforeCount,
          hostsAddedBefore: hostsBeforeCount,
          calibration,
        })
      );
      return calibration;
  ...
  >({
    ...
    // Pre-build O(1) lookup maps instead of O(n) .find()/.filter() per user
    const attributeWeightMap = new Map<number, number>();
    if (attributeWeights) {
      for (const aw of attributeWeights) {
        attributeWeightMap.set(aw.userId, aw.weight);
      }
    }

    const calibrationMap = new Map<number, number>();
    for (const host of allHostsWithCalibration) {
      calibrationMap.set(host.userId, host.calibration);
    }

    // Pre-build booking counts per user: count bookings where user is organizer OR attendee
    const bookingCountByUserId = new Map<number, number>();
    const emailToUserIds = new Map<string, number>();
    for (const user of availableUsers) {
      bookingCountByUserId.set(user.id, 0);
      emailToUserIds.set(user.email, user.id);
    }
    for (const booking of bookingsOfAvailableUsersOfInterval) {
      // Count by organizer userId
      if (booking.userId !== null && bookingCountByUserId.has(booking.userId)) {
        bookingCountByUserId.set(booking.userId, bookingCountByUserId.get(booking.userId)! + 1);
      }
      // Count by attendee email (only if not already counted as organizer)
      for (const attendee of booking.attendees) {
        if (!attendee.email) continue;
        const userId = emailToUserIds.get(attendee.email);
        if (userId !== undefined && userId !== booking.userId) {
          bookingCountByUserId.set(userId, bookingCountByUserId.get(userId)! + 1);
        }
      }
    }
    ...
    const usersWithBookingShortfalls = availableUsers.map((user) => {
      ...
      if (attributeWeights) {
        userWeight = attributeWeightMap.get(user.id) ?? 100;
      }
      const targetPercentage = userWeight / totalWeight;
      const numBookings = bookingCountByUserId.get(user.id) ?? 0;

      const targetNumberOfBookings = (allBookings.length + totalCalibration) * targetPercentage;
      const userCalibration = calibrationMap.get(user.id) ?? 0;
  ...
  public async getOrderedListOfLuckyUsers<AvailableUser extends AvailableUserBase>(
    ...
    // Use a Set for O(1) lookups instead of .map().includes() — avoids O(users² × bookings)
    const remainingUserIds = new Set(availableUsers.map((u) => u.id));
    ...
    // Pre-compute per-user booking counts in O(bookings) instead of O(users × bookings)
    const perUserBookingsCount: Record<number, number> = {};
    for (const booking of bookingsOfAvailableUsersOfInterval) {
      if (booking.userId !== null) {
        perUserBookingsCount[booking.userId] = (perUserBookingsCount[booking.userId] ?? 0) + 1;
      }
    }
    ...
    while (remainingAvailableUsers.length > 0) {
      ...
      remainingUserIds.delete(luckyUser.id);
      remainingAvailableUsers = remainingAvailableUsers.filter((user) => user.id !== luckyUser.id);
      // O(bookings) with Set lookup instead of O(bookings × users) with .map().includes()
