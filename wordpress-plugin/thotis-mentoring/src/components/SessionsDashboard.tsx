...
export function SessionsDashboard({ token: initialToken }: SessionsDashboardProps) {
    ...
    return (
      <div className="th-space-y-6">
        ...
        <div className="th-mx-auto th-max-w-md th-space-y-4">
          <div>
            <label htmlFor="thotis-email-input" className="th-sr-only">
    ...
    return (
      <div className="th-mx-auto th-max-w-md">
        <ErrorBoundary
          ...
          <RatingForm session={ratingSession} token={token} onBack={() => setRatingSession(null)} />
        </ErrorBoundary>
      </div>
    );
  }
  ...
  return (
    <div className="th-space-y-6">
      <h2 className="th-font-heading th-text-2xl th-font-bold th-text-thotis-gray-900">Mes sessions</h2>

      {/* Tabs */}
      <div
        className="th-flex th-gap-1 th-rounded-lg th-bg-thotis-gray-100 th-p-1"
        role="tablist"
      ...
      {activeQuery.error && (
        <div className="th-rounded-lg th-border th-border-red-200 th-bg-red-50 th-p-4 th-text-center">
          <p className="th-text-sm th-text-red-600">Erreur lors du chargement des sessions.</p>
          <button
            type="button"
      ...
      {!activeQuery.isPending && !activeQuery.error && sessions.length === 0 && (
        <div className="th-py-12 th-text-center">
          <p className="th-text-thotis-gray-500">
            Aucune session {tab === "upcoming" ? "à venir" : tab === "past" ? "passée" : "annulée"}
          </p>
          {tab === "upcoming" && (
            <a
