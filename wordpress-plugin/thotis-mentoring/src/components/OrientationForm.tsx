...
export function OrientationForm({ onComplete }: OrientationFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="th-space-y-6">
      ...
      <div>
        <h3 className="th-text-lg th-font-heading th-font-semibold th-text-thotis-gray-900">Tes objectifs</h3>
        <div className="th-mt-3 th-flex th-flex-wrap th-gap-2">
          {GOALS.map((goal) => (
            <label
              ...
              <input type="checkbox" value={goal} {...register("goals")} className="th-sr-only" />
              {goal}
            </label>
          ))}
        </div>
